...
*/
static
void do_shadow_PUTI ( MCEnv* mce, IRStmt *clone, IRPutI *puti)
{
   IRAtom* vatom;
   IRType  ty, tyS;
   Int     arrSize;
...
static
void do_shadow_WRTMP ( MCEnv* mce, IRStmt *clone, IRTemp tmp, IRExpr* expr, HowUsed hu )
{
...
static
void do_shadow_Dirty ( MCEnv* mce, IRStmt* clone, IRDirty* d )
{
   ...
   */
   ...
   for (/*use current i*/; i < sb_in->stmts_used; i++) {
      ...
      switch (st->tag) {
         ...
         case Ist_PutI:
            do_shadow_PUTI( &mce, clone, st->Ist.PutI.details );
            break;
	    ...
	    if (!TNT_(clo_head))
         ...
         case Ist_Dirty:
            do_shadow_Dirty( &mce, clone, st->Ist.Dirty.details );
            break;
...
//                              st->Ist.AbiHint.nia );
   ...
   create_dirty_NEXT( &mce, sb_in->next );
